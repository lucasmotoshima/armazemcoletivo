...
class Cart_controller extends CI_Controller
{
	...
	public function getPedidos(){
		...
		if(isset($_REQUEST['email'])){
			if($_REQUEST['email']!=''){
				$params 				= array('email'=>$_REQUEST['email'],'code'=>$_REQUEST['code'],'active'=>'1');
				$provider 				= $this->Provider_model->getProvider($params);
				//$this->debug->show(count($provider));
				//$this->debug->show();
				$data['provider']		= $provider;
				
				if( (count($provider) > 0) and ($_REQUEST['email'] = preg_replace( array( '/[ ]/' , '/[^A-Za-z0-9\-]/' ) , array( '' , '' ) , $_REQUEST['phone'] )) ){
					...
					$deliveryTime 			= 0;
					$_REQUEST['email']		= $provider[0]['email'];
