...
class Cart_controller extends CI_Controller
{
	...
	public function add()
	{
		...
		if((count($cart[0])==0) or ($cart[0]['status']!='W'))
		{
			$dados = array(
				'session_id'    => $session_id,
			    'id'      		=> $id,
			    'price'			=> $calc_total['product_original_sale_price'] + $calc_total['print_unit_total'],
			    'name'    		=> (isset($product[0]['name']))?$product[0]['name']:NULL,
		...
		if($calc_total['product_original_price']!=0){
			...
			else
			{
				...
				$data['line']		.= '<td>'.'P'.str_pad($product[0]['id'], 4, "0", STR_PAD_LEFT).' - '.$product[0]['name'].'</td>';
