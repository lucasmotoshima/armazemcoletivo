...
class cart_model extends CI_Model {
	...
	public $messages = array();
  	...
  	{
		if($pkValue==null) 
		 	$query = $this->db->get_where('cart', array('session_id'=>$pk));
		else
	...
	public function set($dados)
	{
		...
		(isset($dados['phone']))?$this->db->set('phone','('.$ddd.') '.$phone):'';
...
//=================================================================
	...
	public function getList($dados=null)//
	{
		...
		if(isset($dados['ciudad']))
			$this->db->like('cart.ciudad',$dados['ciudad']);
		if(isset($dados['status']))
			$this->db->like('cart.status',$dados['status']);
