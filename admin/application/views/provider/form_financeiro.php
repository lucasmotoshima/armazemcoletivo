...
<script type="text/javascript">
    ...
    $(document).ready( function() {
		...
		$('#icoAdd').click(function(){
			...
			jQuery.ajax({
				type: "POST",
				url: '<?=base_url('plan/sendBoletoPagseguro')?>',
				data: {
					id : id,
					plans : plans,
					months : months,
					        ...
					        <script type="text/javascript">
					            $(function () {
						            ...
						            $('#months_installment').on('change', function(e){
										...
										price_total 			= price_total.toLocaleString('pt-br',{style: 'currency', currency: 'BRL'});
										var amountPerMonth		= price_total / parseInt(months_installment);
										$('#price_month').val(amountPerMonth.toLocaleString('pt-br',{style: 'currency', currency: 'BRL'}));
						            });
							...
							<div id='addIndustry'>
								<!--form action="<?=base_url('provider/addPlan/'.$provider[0]['id'])?>" method="post" accept-charset="utf-8" enctype="multipart/form-data" id="providerPlanForm"-->
									...
									<?if(count($provider['providerPlan'])>0){?>
										<?foreach ($provider['providerPlan'] as $key => $row) {?>
											<table id='provider_plan<?=$row['id']?>' class="table">
												<tr>
													<td align="center" colspan="5">
														<strong><u><?=$row['id']?> - <?=$row['name']?></u></strong>
													</td>
													<td colspan="4" >
