...
class User_model extends CI_Model {
	...
	public function checkLogin() {
		if (isset($_SESSION['adm_armazem']['user']['id'])) {//possui conteudo na session?
			$this->id 			= $this->id != null ? $this->id : $_SESSION['adm_armazem']['user']['id'];
			$this->email 		= $this->email != null ? $this->email : $_SESSION['adm_armazem']['user']['email'];
			$this->password 	= $this->senha != null ? $this->password : $_SESSION['adm_armazem']['user']['password'];
		}
		return $this->doLogin();
	}

	public function registerLoginSession($resultado) {
		$_SESSION['adm_armazem']['user']['id'] 			= $resultado['id'];
		$_SESSION['adm_armazem']['user']['name'] 		= $resultado['name'];
		$_SESSION['adm_armazem']['user']['email'] 		= $resultado['email'];
		$_SESSION['adm_armazem']['user']['password'] 	= $resultado['password'];
		$_SESSION['adm_armazem']['user']['type'] 		= $resultado['type'];
	}
	...
	public function save($dados)
	{
		...
		{
			$password						= (isset($dados['password'])?md5($dados['password']):'');
