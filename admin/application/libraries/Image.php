...
class image {
	...
	function get_listPhotos($code,$w,$h) {
		...
		for ($i=1; $i <= 4; $i++) {
			...
			if (file_exists($filename)){
				...
				$fotoList[$i]['url'] 		= base_url('public/images/product/'.$code.'_'.$i.'.jpg');
				$fotoList[$i]['download'] 	= base_url('product/download/'.$code.'/'.$i);
	...
	public function resize($code,$maxW,$maxH) {
		$filename 	= base_url('public/images/product/'.$code.'.jpg');
